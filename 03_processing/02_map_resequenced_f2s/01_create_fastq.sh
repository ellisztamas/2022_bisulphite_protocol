...
# Script to create fastq files that can be passed to methylseq
...
#SBATCH --mem=40gb
#SBATCH --array=0-99
#SBATCH --output=./slurm/create_fastq.%a.out
#SBATCH --error=./slurm/create_fastq.%a.err
...
read_pair_1=(01_data/12_resequenced_f2s/*_1#*bam)
read_pair_2=(01_data/12_resequenced_f2s/*_2#*bam)
...
workdir=/scratch-cbe/users/$(whoami)/mix_plate

# Merge files.
# Swap file path and extension for output file
merged_bam=${infile1/'01_data/12_resequenced_f2s'/"$workdir/merged_bams"}
...
fastq1=${infile1/'01_data/12_resequenced_f2s'/"$workdir/fastq"}
fastq2=${infile2/'01_data/12_resequenced_f2s'/"$workdir/fastq"}
