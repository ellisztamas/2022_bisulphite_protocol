...
```{r}
# estimate of the non-conversion error rate from the chloroplast
lambda <- 0.072
...
methylation_in_windows <- read_csv(
  "05_results/13_identifying_te_methylation/output/mC_in_windows.csv",
  col_types = 'ccciii'
) %>% 
  rename(id = start)


col0_paths <- Sys.glob("05_results/15_nonconversion_uncertainty/output/*.methylation_counts.csv")
...
```{r}
state_cmt2 <- cmt2 %>% 
  call_methylation_state(shape1=17, shape2=220)
state_rddm <- rddm %>% 
  call_methylation_state(shape1=17, shape2=220)
...
```{r}
state_col0 <- vector('list', 4)

# Estimate beta parameters for error rates for each col0 genome
for(i in 1:4){
state_col0[[i]] <- col0_in_windows[[i]] %>% 
  # filter(chr == "Chr5") %>% 
  filter(chr %in% paste0("Chr", 1:5)) %>%
  call_methylation_state(
    shape1 = beta_params[[i]]$a,
    shape2 = beta_params[[i]]$b
    )
}

# How often do methylation state calls match?
# about 70% of the time, depending what you compare
for(i in 1:3){
  for(j in 2:4){
    if(i < j) cat(i, j, mean(state_col0[[i]]$state == state_col0[[j]]$state, na.rm = TRUE), "\n")
  }
}
