...
#'
#' @author  Tom Ellis
call_methylation_state <- function(read_counts, shape1, shape2, return_probabilities=FALSE){

  # Total coverage
  read_counts$n <- read_counts$meth + read_counts$unmethylated

  # Mean methylation, without correcting for errors
  read_counts$theta <- read_counts$meth / read_counts$n
  ...
  ll_binom <- dbinom(read_counts$meth, size = read_counts$n, prob = read_counts$theta, log = TRUE)
